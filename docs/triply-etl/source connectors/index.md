...
Source Connectors allow one or more backend systems to be connected to a TriplETL pipeline.
...
We use `logRecord()` to print the current record to standard output.  When we run this pipeline, the two records are printed as follows:
...
Notice that TriplyETL adds two keys to both records: `$recordId` and `$environment` (see [Production Pipelines](production-pipelines)).
