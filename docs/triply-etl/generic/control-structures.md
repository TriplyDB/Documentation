...
Notice that [forEach()](#iterating-over-lists-of-objects-foreach) can only iterate over Records. Sometimes, it is necessary to iterate over primitive values, for example strings. If the iteration directly results in RDF assertions, [iris()](../assert/ratt/terms.md#iris) and [literals()](../assert/ratt/terms.md#literals) can be used. But in other cases, we must map the primitive values to objects. These objects can then be processed as regular Records.
