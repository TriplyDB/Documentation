...
With the `ifElse()` function in TriplyETL it's possible to specify multiple conditions based on which other functions are run.
...
**Example 2**: When ETLs transform different kinds of entities, it can be useful run a sub-ETL based on the type of entity.
...
This is a great use case for `_switch()`:
