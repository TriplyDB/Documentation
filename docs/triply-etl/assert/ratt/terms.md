...
- `prefix` A prefix that is declared with [declarePrefix()](../../generic/declarations.md#declarePrefix).
...
If the same IRI is used in multiple statements, repeating the same assertion multiple times may impose a maintenance burden. In such cases, it is possible to first add the IRI to the record using transformation function [addIri()](../../transform/ratt.md#function-addiri), and refer to that one IRI in multiple statements.
...
This function allows us to create ordered closed collections [rdf:List](https://triplydb.com/how-to-model/-/stories/collections) in TriplyETL. The elements of such collections must be represented using a single-linked list (`rdf:first` and `rdf:rest`). Such collections are closed by the empty list (`rdf:nil`).


### Parameters

- `prefix` A prefix for linked lists that is declared with [declarePrefix()](../../generic/declarations.md#declarePrefix).
...
If the same literal is used in multiple statements, repeating the same assertion multiple times can impose a maintenance burden. In such cases, it is possible to first add the literal to the record with transformation [addLiteral()](../../transform/ratt.md#function-addliteral), and refer to that one literal in multiple statements.
